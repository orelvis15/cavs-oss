...
| apply output byte-identical | PASS | verified against new signature in 225 ms (121.38 MiB from old, 5.51 MiB fresh) |
| no-op reapply | PASS | 0 files rewritten (44 detected as no-op); output byte-identical |
| corruption smoke checks | SKIPPED | strict profile only (or `cavs certify integrity`) |
...
Detailed report: `regressions.md`
